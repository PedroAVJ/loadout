...
- If already running inside a `codex review` session, do not invoke `codex review` again. Perform a manual code-review pass over the provided diff/files instead and report findings directly. Recursive review sessions can hang and block the parent worker.
